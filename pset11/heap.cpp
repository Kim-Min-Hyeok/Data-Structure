...
void trim(heap p) {
	...
	::swap(p, 1, p->N--);
	sink(p, 1);
	if(p->N <= capa/4) reserve(p, capa/2);
...
void sink(heap h, int k) {
	DPRINT(cout << " sink key=" << h->nodes[k] << " k=" << k << " N=" << h->N << endl;);

	while(2 * k <= h->N) {
		int j=2*k;
		if(j<h->N && h->comp(h,j,j+1)) j++;
		if(!h->comp(h,k,j)) break;
		::swap(h,k,j);
		k=j;
	}

#ifdef DEBUG
	cout << "\tafter sink N=" << h->N << " k=" << endl;
	for (k = 1; k <= h->N; k++)
		cout << h->nodes[k] << " ";
	cout << endl;
#endif
...
bool heapOrderedAt(heap p, int k) {
	...
	if ((left  >= p->N)||(right >= p->N)) return true;
...
// if maxheap, it sorts them in ascending once
...
// 3. Allocate a type array such as keys to store random keys.
