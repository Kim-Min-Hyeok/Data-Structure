...
Node* push_frontN(Node* p, int N) {
	...
	for(int i=0; i<N; i++){
		p = push_front(p, rand_extended(range)%range);
	}
...
pNode push_backN(pNode p, int N) {
	...
	for(int i=0; i<N; i++){
		p = push_back(p, rand_extended(range)%range);
	}
...
Node* push_N(Node* p, int N, Node* (*push_fp)(Node*, int)) {
	...
	for(int i=0; i<N; i++){
		p = push_fp(p, rand_extended(range)%range);
	}
...
Node* reverse_odd2(Node* head) {
	...
	while (head != nullptr) {
		...
		while (!ostack.empty()) {
			...
			head2_data	= ostack.top();
			ostack.pop();
			head2 = push_back(head2, head2_data);
		}
	...
	while(head != nullptr) {
		...
		while(!ostack.empty()){
		  temp = ostack.top();
			ostack.pop();
			if(head2 == nullptr) {
				head2 = new Node{temp->data, nullptr};
				tail2 = head2;
			}
			else {
				tail2->next = new Node{temp->data, nullptr};
				tail2 = tail2->next;
			}
		}
		if(head2 == nullptr) {
			head2 = new Node{head->data, nullptr};
			tail2 = head2;
		}
		else {
			tail2->next = new Node{head->data, nullptr};
			tail2 = tail2->next;
		}
		head = head->next;
	}
...
void show(pNode p, bool all, int show_n) {
	...
	// print the first show_n items
		...
		for (i = 1, curr = p; i<=show_n; curr = curr->next, i++){
			cout << " -> " << curr->data;
		}
	...
	for(i=show_n; i<size(p)-show_n; i++){
		curr = curr->next;
	}

	for (i = 1; i<=show_n; curr = curr->next, i++){
		cout << " -> " << curr->data;
	}
