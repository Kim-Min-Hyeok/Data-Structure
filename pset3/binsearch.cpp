...
int _binary_search(int *data, int key, int lo, int hi) {
	...
	if( key > data[mi] && key < data[mi+1]) return -(mi+1)-1;
	if(key < data[mi])
		return _binary_search(data, key, lo, mi - 1);
	else
		return _binary_search(data, key, mi + 1, hi);
}
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < size; i++) {
		int key = (rand()% list[size - 1]) + list[0];
		cout << "\t" << key;
		if(binary_search(list, key, size)>=0)
			cout << "\tis @[" <<binary_search(list, key, size)<< "]" << endl;
    else
			cout << "\tis not @[" <<-(binary_search(list, key, size)+1)<< "]" << endl;
	}
