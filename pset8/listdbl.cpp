...
pNode half(pList p) {  // method 2 - rabbit and turtle
	...
	while (rabbit != end(p)) {
		if(rabbit->next == end(p)) rabbit = rabbit->next;
...
pNode temp =nullptr;
	for(pNode c = begin(p); c != end(p); c=temp){
		temp = c->next;
...
void push_backN(pList p, int N) {
	...
	for(int i=0; i<N; i++) push_back(p, rand_extended(range)%range);
...
void reverse(pList p) {
	...
	pNode temp = nullptr;
	for(pNode curr = p->head; curr != nullptr; curr = temp){
		temp = curr->next;
...
void shuffle(pList p) {
	...
	pNode mid = half(p);
	pNode que = begin(p);
	pNode que_last = mid->prev;
	que_last->next = nullptr;
	que->prev = nullptr;
	p->head->next = mid;
	mid->prev = p->head;
	mid = begin(p)->next;
...
bool sorted(pList p, int(*comp)(int a, int b)) {
	...
	pNode c = begin(p);
	for (; c->next != end(p); c = c->next)
		if (comp(c->next->data, c->data)<0) return false;
...
void push_sortedN(pList p, int N) {
	...
	for (int i=0; i < N; i++) {
		int val = rand_extended(range) % range;
		if(s) insert(more(p,val), val);
		else insert(::less(p,val), val);
	}
#endif
...
// The values for new nodes are randomly generated in the range of
// [0..(N + size(p))). You may use rand_extended().
int partition_arr(int list[], int lo, int hi, int(*comp)(int, int) = ascending) {
	int x = list[hi];  // pivot
	int i = (lo - 1);  // Index of smaller element

	for (int j = lo; j <= hi - 1; j++) {
		if (comp(x, list[j]) > 0) { 				// if current element is <= pivot
...
void _quickSort(int *list, int lo, int hi, int(*comp)(int, int) = ascending) {
	if (lo < hi) 	{
		int pi = partition_arr(list, lo, hi, comp); // Partitioning index
		_quickSort(list, lo, pi - 1, comp);
		_quickSort(list, pi + 1, hi, comp);
	}
}

void quickSort_arr(int *a, int n, int(*comp)(int, int) = ascending) {
	_quickSort(a, 0, n - 1, comp);  // the last argument n is added only for DPRINT()
}
void push_sortedNlog(pList p, int N) {
	...
	int* vals = new int[N];
	pNode curr = begin(p);
	int sorted = last(p)->data - curr->data;

	for(int i=0; i<N; i++){
		vals[i] = rand_extended(range) % range;
	}

	quickSort_arr(vals, N);
	int i = 0;
	while(i<N){
		if(curr != end(p)){
		if(sorted>0){//오름차순
